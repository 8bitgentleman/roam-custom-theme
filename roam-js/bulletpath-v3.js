...
// BULLET
let scale = 'var(--bulletPath-BulletScale)';
let scaleClosed = 'var(--bulletPath-BulletScaleClosed)';
let bulletColor = 'var(--bulletPath-BulletColor)';

// LINES
let showLines = true;
let lineWidth = 2;
let lineColor = 'var(--bulletPath-LineColor)';
let borderRadius = 5;

// HIGHLIGHT REFS
let highlightRefs = true;
let refColor = 'var(--bulletPath-RefColor)';
...
.path-highlighted .rm-bullet .rm-bullet__inner--numbered{
	color: ${bulletColor} !important;
    transform: scale(${scale});
}
.path-highlighted .rm-bullet--closed .rm-bullet__inner {
    background-color: ${bulletColor} !important;
    transform: scale(${scaleClosed}) ;
}
`
if (highlightRefs) {
    baseStyle += `
...
.path-highlighted + .roam-block .rm-alias {
	color: ${refColor} !important;
    text-decoration: none !important;
}
.path-highlighted + .roam-block .rm-block-ref {
	/*border-bottom-color: ${refColor} !important;*/
	padding: 3px 7px !important;
    font-weight: 500 !important;
    border-radius: 4px !important;
	border-color:${refColor} !important;
	border: 1px solid ${refColor} !important;
}
`
}
...
function ppkeypress(e) {
    if (e.ctrlKey && e.key === 'i') {
        ...
        if (pptoggle) {
            ...
            let bullets = [];
            let elt = document.querySelectorAll('textarea.rm-block-input');
            let block = elt[0].closest('.roam-block-container');
            ...
            bullets.forEach(function(bullet, i) {
                ...
                if (!bullet.dataset.pathidentifier) {
                    bullet.dataset.pathidentifier = pathid++
                }
...
document.addEventListener('keypress', ppkeypress, false);
